...
const countReasources = (reasources) => {
    const values = Object.values(reasources);
...
const updateReasources = (name, reasourceChange, game) => {
    const player = game.hands.find(hand => hand.name == name);
    if (player) {
...
const processBuyMsg = async (ele) => {
    const reasourceChange = getBlankReasourcesItem();
    const span = ele.querySelector('.semibold');
    const info = ele.querySelector('span');
    if (span && info) {
        const name = span.textContent;
        const image = info.querySelector('img');
        if (image) {
...
const processBankTradeMsg = async (ele) => {
    const reasourceChange = getBlankReasourcesItem();
    const span = ele.querySelector('.semibold');
    const innerSpan = ele.querySelector('span');
    if (span && innerSpan) {
        const name = span.textContent;
...
const processMonopoly = async (span, nameA) => {
    var val = -1;
    var num = -1;
    const reasourceChange = getBlankReasourcesItem();
...
const processPlayerTrade = async (ele) => {
    const reasourceChangeA = getBlankReasourcesItem();
    const reasourceChangeB = getBlankReasourcesItem();
    const span = ele.querySelectorAll('.semibold');
    const innerSpan = ele.querySelector('span');
    const nameA = span[0].textContent;
    const nameB = span[1].textContent;
    if (span && innerSpan) {
        //console.log('name' , name);
        let giving = true;
        innerSpan.childNodes.forEach(node => {
...
const processStealMsg = async (ele) => {
    const reasourceChangeA = getBlankReasourcesItem();
    const reasourceChangeB = getBlankReasourcesItem();
    ...
    const game = await getGame();
    innerSpan?.childNodes.forEach(node => {
        if (node.className == "semibold" || (node.nodeValue == "You stole " || node.nodeValue == " from you")) {
            if (!nameA) {
                if (node.className == "semibold") {
...
const processStartingMessage = async (ele) => {
    ...
    if (name) {
        ...
        if (game) {
            ...
            const nameSet = new Set(hands.map(hand => hand.name));
            if (true) {
                const newHand = {
                    name,
...
const processGotMsg = async (ele, saveReasources = true) => {
    const reasourceChange = getBlankReasourcesItem();
    const span = ele.querySelector('.semibold');
    const info = ele.querySelector('span');
    if (span && info) {
        const name = span.textContent;
        const images = info.querySelectorAll('img');
        images.forEach(img => {
            if (img.alt == 'ore') {
...
const processDiscardMsg = async (ele) => {
    const reasourceChange = getBlankReasourcesItem();
    const span = ele.querySelector('.semibold');
    const info = ele.querySelector('span');
    if (span && info) {
...
const processMessage = async (ele) => {
    //console.log(ele);
...
const checkQChange = async (previousSize, queue) => {
    while (queue.length > 0) {
        const first = queue.shift();
        await processMessage(first);
    }
};
const observeQueue = async (queue) => {
...
const processAllMessages = () => {
    ...
    const messages = document.querySelectorAll(".message-post");
    messages.forEach(processMessage);
};
// Content script
// Callback function to handle mutations
function handleMutations(mutations, observer, queue) {
    mutations.some((mutation) => {
        const addedElement = mutation.target;
...
const startMutationObserver = () => {
    ...
    const observer = new MutationObserver((h, m) => {
...
const stopMutationObserver = (observer) => {
    observer.disconnect();
};
function processDocument() {
    processAllMessages();
}
const checkIfInGame = (url) => {
    var pattern = /#\w{4}$/;
